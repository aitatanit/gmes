...
namespace gmes
{
  ...
  {
  public:
    DCPEx(double epsilon,
	  const double * const a, int a_i_size, int a_j_size,
	  const double * const b, int b_i_size, int b_j_size,
	  const double * const c, int c_size):
      DCPElectric<T>(epsilon, a, a_i_size, a_j_size, b, b_i_size, b_j_size, c, c_size)
    {
    }
  ...
  {
  public:
    DCPPLRCElectric(double epsilon,
		    const double * const a, int a_i_size, int a_j_size,
		    const std::complex<double> * const b_c, 
		    int b_c_i_size, int b_c_j_size,
		    const double * const c, int c_size):
      eps(epsilon),
      c(c, c + c_size),
      ...
      for (int i = 0; i < b_c_i_size; i++)
	{
	  std::vector<std::complex<double> > 
	    tmp(b_c + i * b_c_j_size, b_c + (i + 1) * b_c_j_size);
	  this->b.push_back(tmp);
	}
    }
  ...
  {
  public:
    DCPPLRCEx(double epsilon,
	      const double * const a, int a_i_size, int a_j_size,
	      const std::complex<double> * const b_c, int b_c_i_size, int b_c_j_size,
	      const double * const c, int c_size):
      DCPPLRCElectric<T>(epsilon, a, a_i_size, a_j_size, b_c, b_c_i_size, b_c_j_size, c, c_size)
    {
    }
		...
		double dy, double dz, double dt, double n, int i, int j, int k)
    {
      ...
      std::complex<double> e_new = (c[0] * e_now
				    + c[1] * ((hz(i+1,j+1,k) - hz(i+1,j,k)) / dy - 
					      (hy(i+1,j,k+1) - hy(i+1,j,k)) / dz)
				    + c[2] * psi_total());
  ...
  public:
		...
		double dz, double dx, double dt, double n, int i, int j, int k)
    {
      ...
      std::complex<double> e_new = (c[0] * e_now + c[1] 
				    * ((hx(i,j+1,k+1) - hx(i,j+1,k)) / dz - 
				       (hz(i+1,j+1,k) - hz(i,j+1,k)) / dx)
				    + c[2] * psi_total());
  ...
  public:
		...
		double dx, double dy, double dt, double n, int i, int j, int k)
    {
      ...
      std::complex<double> e_new = (c[0] * e_now
				    + c[1] * ((hy(i+1,j,k+1) - hy(i,j,k+1)) / dx - 
					      (hx(i,j+1,k+1) - hx(i,j,k+1)) / dy)
				    + c[2] * psi_total());
