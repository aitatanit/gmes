...
class TransparentElectric:
    def __init__(self, pw_material, epsilon_r, aux_fdtd):
        self.idx = pw_material.i, pw_material.j, pw_material.k
...
class TransparentMinusYEx(TransparentElectric):
    ...
    def update(self, ex, hz, hy, dt, dy, dz):
        self.pw_material.update(ex, hz, hy, dt, dy, dz)
        ex[self.idx] += dt / (self.epsilon * dy) * self.aux_fdtd.hy[1,0,11]
        self.aux_fdtd.step()
...
class TransparentMinusYEz(TransparentElectric):
    ...
    def update(self, ez, hy, hx, dt, dx, dy):
        self.pw_material.update(ez, hy, hx, dt, dx, dy)
        ez[self.idx] -= dt / (self.epsilon * dy) * self.aux_fdtd.hy[1,0,11]
        self.aux_fdtd.step()
...
class TransparentPlusZEx(TransparentElectric):
    ...
    def update(self, ex, hz, hy, dt, dy, dz):
        self.pw_material.update(ex, hz, hy, dt, dy, dz)
        ex[self.idx] += dt / (self.epsilon * dz) * self.aux_fdtd.hy[1,0,12]
        self.aux_fdtd.step()
        
        
class TransparentPlusZEy(TransparentElectric):
    def __init__(self, pw_material_ey, epsilon_r, amp, aux_fdtd):
        TransparentElectric.__init__(self, pw_material_ey, epsilon_r, aux_fdtd)
        
    def update(self, ey, hx, hz, dt, dz, dx):
        self.pw_material.update(ey, hx, hz, dt, dz, dx)
        ey[self.idx] -= dt / (self.epsilon * dz) * self.aux_fdtd.hy[1,0,12]
        self.aux_fdtd.step()
...
class TransparentMinusZEx(TransparentElectric):
    ...
    def update(self, ex, hz, hy, dt, dy, dz):
        self.pw_material.update(ex, hz, hy, dt, dy, dz)
        ex[self.idx] -= dt / (self.epsilon * dz) * self.aux_fdtd.hy[1,0,11]
        self.aux_fdtd.step()
...
class TransparentMinusZEy(TransparentElectric):
    ...
    def update(self, ey, hx, hz, dt, dz, dx):
        self.pw_material.update(ey, hx, hz, dt, dz, dx)
        ey[self.idx] += dt / (self.epsilon * dz) * self.aux_fdtd.hy[1,0,11]
        self.aux_fdtd.step()
...
class TransparentPlusXEy(TransparentElectric):
    ...
    def update(self, ey, hx, hz, dt, dz, dx):
        self.pw_material.update(ey, hx, hz, dt, dz, dx)
        ey[self.idx] += dt / (self.epsilon * dx) * self.aux_fdtd.hy[1,0,12]
        self.aux_fdtd.step()
...
class TransparentPlusXEz(TransparentElectric):
    ...
    def update(self, ez, hy, hx, dt, dx, dy):
        self.pw_material.update(ez, hy, hx, dt, dx, dy)
        ez[self.idx] -= dt / (self.epsilon * dx) * self.aux_fdtd.hy[1,0,12]
        self.aux_fdtd.step()
...
class TransparentMinusXEy(TransparentElectric):
    ...
    def update(self, ey, hx, hz, dt, dz, dx):
        self.pw_material.update(ey, hx, hz, dt, dz, dx)
        ey[self.idx] -= dt / (self.epsilon * dx) * self.aux_fdtd.hy[1,0,11]
        self.aux_fdtd.step()
...
class TransparentMinusXEz(TransparentElectric):
    ...
    def update(self, ez, hy, hx, dt, dx, dy):
        self.pw_material.update(ez, hy, hx, dt, dx, dy)
        ez[self.idx] += dt / (self.epsilon * dx) * self.aux_fdtd.hy[1,0,11]
        self.aux_fdtd.step()
        
        
class TransparentMagnetic:
    def __init__(self, pw_material, mu_r, aux_fdtd):
        self.idx = pw_material.i, pw_material.j, pw_material.k
        self.pw_material = pw_material
        self.mu = mu_r * mu0
        self.aux_fdtd = aux_fdtd
...
class TransparentMinusYHz(TransparentMagnetic):
    ...
    def update(self, hz, ey, ex, dt, dx, dy):
        self.pw_material.update(hz, ey, ex, dt, dx, dy)
        self.aux_fdtd.step()
        hz[self.idx] += dt / (self.mu * dy) * self.aux_fdtd.ex[0,0,10]
...
class TransparentPlusZHx(TransparentMagnetic):
    ...
    def update(self, hx, ez, ey, dt, dy, dz):
        self.pw_material.update(hx, ez, ey, dt, dy, dz)
        self.aux_fdtd.step()
        hx[self.idx] -= dt / (self.mu * dz) * self.aux_fdtd.ex[0,0,12]
...
class TransparentMinusZHy(TransparentMagnetic):
    ...
    def update(self, hy, ex, ez, dt, dz, dx):
        self.pw_material.update(hy, ex, ez, dt, dz, dx)
        self.aux_fdtd.step()
        hy[self.idx] -= dt / (self.mu * dz) * self.aux_fdtd.ex[0,0,10]
...
class TransparentMinusZHx(TransparentMagnetic):
    ...
    def update(self, hx, ez, ey, dt, dy, dz):
        self.pw_material.update(hx, ez, ey, dt, dy, dz)
        self.aux_fdtd.step()
        hx[self.idx] += dt / (self.mu * dz) * self.aux_fdtd.ex[0,0,10]
...
class TransparentPlusXHz(TransparentMagnetic):
    ...
    def update(self, hz, ey, ex, dt, dx, dy):
        self.pw_material.update(hz, ey, ex, dt, dx, dy)
        self.aux_fdtd.step()
        hz[self.idx] += dt / (self.mu * dx) * self.aux_fdtd.ex[0,0,12]
...
class TransparentPlusXHy(TransparentMagnetic):
    ...
    def update(self, hy, ex, ez, dt, dz, dx):
        self.pw_material.update(hy, ex, ez, dt, dz, dx)
        self.aux_fdtd.step()
        hy[self.idx] -= dt / (self.mu * dx) * self.aux_fdtd.ex[0,0,12]
...
class TransparentMinusXHz(TransparentMagnetic):
    ...
    def update(self, hz, ey, ex, dt, dx, dy):
        self.pw_material.update(hz, ey, ex, dt, dx, dy)
        self.aux_fdtd.step()
        hz[self.idx] -= dt / (self.mu * dx) * self.aux_fdtd.ex[0,0,10]
...
class TransparentMinusXHy(TransparentMagnetic):
    ...
    def update(self, hy, ex, ez, dt, dz, dx):
        self.pw_material.update(hy, ex, ez, dt, dz, dx)
        self.aux_fdtd.step()
        hy[self.idx] += dt / (self.mu * dx) * self.aux_fdtd.ex[0,0,10]
