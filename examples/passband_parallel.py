...
"""
This script can handle parallel execution and data-collection. The
fields in the waveguide are recorded following probe lines. Each 
field data is pickled and saved in separate files.
...
AIR = Dielectric(1)
Al2O3 = Dielectric(8.9)


def make_rod(center):
    """Stand a rod."""

    return Cylinder(material=Al2O3, axis=(0,0,1), radius=RADIUS,
...
def make_highpass_bragg_grating(period):
    ...
    for i in arange(-(.5 * period - .5) * l, (.5 * period + .5) * l, l):
        rod = Cylinder(material=Al2O3, axis=(0,0,1), radius=0.0623302*l,
...
def make_lowpass_bragg_grating(period):
    ...
    for i in arange(-(.5 * period - .5) * l, (.5 * period + .5) * l, l):
        rod = Cylinder(material=Al2O3, axis=(0,0,1), radius=0.0362054*l,
                       height=10, center=(i, 0, 0))
        bragg.append(rod)
...
if __name__ == "__main__":
    ...
    src_list = (PointSource(src_time=Bandpass(freq=FREQ, 
                                              fwidth=FWIDTH),
                            component=Ez, 
    ...
    my_fdtd.init()
    my_fdtd.show_permittivity_ez(Z, 0)
    my_fdtd.show_ez(Z, 0)
